...
final class MdlCardViewModel: DocumentModel {
    ...
    let docType: DocumentAttributesProtocol? = DocType.mdlDocument
    ...
    private func deleteDocument(in view: BaseView, force: Bool = false) {
        ...
        Task {
            let docType = model?.docData.docType ?? EudiWalletDocumentType.mdl.rawValue
            try await EudiWalletKitController.instance.deleteDocuments(with: docType)
            await MainActor.run {
                DocumentsProcessor.instance.refreshDocumentFromStorage(type: DocType.mdlDocument.rawValue)
...
// MARK: - Constants
private extension MdlCardViewModel {
    enum Constants {
