...
final class PidCardViewModel: DocumentModel {
    ...
    let docType: DocumentAttributesProtocol? = DocType.pidDocument
    ...
    private func deleteDocument(in view: BaseView, force: Bool = false) {
        ...
        Task {
            try await EudiWalletKitController.instance.clearAllDocuments()
            try await EudiWalletKitController.instance.loadDocuments()
            await MainActor.run {
                DocumentsProcessor.instance.refreshDocumentFromStorage(type: DocType.pidDocument.rawValue)
...
// MARK: - Constants
private extension PidCardViewModel {
    enum Constants {
